...
if (user_input != st.session_state.last_input or user_input_ph != st.session_state.last_input_ph) or st.session_state.rs_val_flag == True:
    ...
    if st.session_state.flag == True and (st.session_state.rs_val_flag == False or option_box != st.session_state.selected_option):
        ...
        Explain these diseases concisely, announce if a disease has been refuted, no need to explain that disease.if no diseases found reply with: No linked diseases found based on the ClinGen Gene-Disease database. 
        ...
        Analyze the abstracts of the papers and draw a conclusion on if the variant is likely to cause the mentioned disease (phenotype) or not.
